...
class BounceDetector:
    ...
    def prepare_features(self, x_ball, y_ball):
        ...
        -------
        features : DataFrame
            features prepared for CatBoost model
        frames : list
            list of frame numbers corresponding to features
        """
    ...
    def predict(self, x_ball, y_ball, smooth=True):
        """
        Predict the frames where the ball bounces based on its trajectory.

        Parameters
        ----------
        x_ball : list of float
            List of x coordinates of the ball in each frame.
        y_ball : list of float
            List of y coordinates of the ball in each frame.
        smooth : bool, optional
            Whether to smooth the ball trajectory before prediction, by default True.

        Returns
        -------
        set of int
            Set of frame indices where the ball is predicted to bounce.
        """
        if smooth:
    ...
    def smooth_predictions(self, x_ball, y_ball):
        """
        Smooth out ball predictions by extrapolating missing points.
        
        Parameters
        ----------
        x_ball, y_ball: lists of x and y coordinates of ball predictions
        
        Returns
        -------
        x_ball, y_ball: lists of x and y coordinates of ball predictions with missing points extrapolated
        """
    ...
    def extrapolate(self, x_coords, y_coords):
        """
        Extrapolates the next point in a sequence of coordinates using cubic splines.

        :param x_coords: list of x-coordinates
        :type x_coords: list of float
        :param y_coords: list of y-coordinates
        :type y_coords: list of float
        :return: extrapolated x and y coordinates
    ...
    def postprocess(self, ind_bounce, preds):
        """
        Postprocess bounce predictions by removing bounces that are too close
        and keep the one with higher probability
        
        :param ind_bounce: indices of bounce predictions
        :type ind_bounce: list of int
        :param preds: the corresponding probabilities of bounce predictions
        :type preds: list of float
        :return: filtered bounce predictions
        :rtype: list of int
        """
